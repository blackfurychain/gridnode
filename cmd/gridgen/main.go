...
import (
	...
	"github.com/Gridironchain/gridnode/tools/gridgen"
	"github.com/spf13/cobra"
)

func main() {
	rootCmd := &cobra.Command{Use: "gridgen"}
...
func networkCreateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			network := gridgen.NewGridgen(&args[0]).NewNetwork(keyringBackend)
...
func networkResetCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			gridgen.NewGridgen(&args[0]).NetworkReset(args[1])
		},
	}
}
...
func nodeCreateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			node := gridgen.NewGridgen(&args[0]).NewNode(keyringBackend)
			node.Moniker = args[1]
...
func nodeResetStateCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			if len(args) == 1 {
				gridgen.NewGridgen(&args[0]).NodeReset(nil)
			} else {
				gridgen.NewGridgen(&args[0]).NodeReset(&args[1])
			}
		},
	}
}
...
func keyGenerateMnemonicCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.MaximumNArgs(0),
		Run: func(cmd *cobra.Command, args []string) {
			gridgen.NewGridgen(nil).KeyGenerateMnemonic("", "")
		},
	}
}
...
func keyRecoverFromMnemonicCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.MinimumNArgs(1),
		Run: func(cmd *cobra.Command, args []string) {
			gridgen.NewGridgen(nil).KeyRecoverFromMnemonic(args[0])
		},
	}
}
