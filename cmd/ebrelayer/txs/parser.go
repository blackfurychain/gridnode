...
import (
	...
	ethbridge "github.com/Gridironchain/gridnode/x/ethbridge/types"
)

const (
	nullAddress           = "0x0000000000000000000000000000000000000000"
	defaultGridironchainPrefix = "c"
)
...
func EthereumEventToEthBridgeClaim(valAddr sdk.ValAddress, event types.EthereumEvent, symbolTranslator *symbol_translator.SymbolTranslator, sugaredLogger *zap.SugaredLogger) (ethbridge.EthBridgeClaim, error) {
	...
	case ethbridge.ClaimType_CLAIM_TYPE_BURN:
		symbol = symbolTranslator.EthereumToGridironchain(symbol)
	}
...
func BurnLockEventToCosmosMsg(claimType types.Event, attributes []abci.EventAttribute, symbolTranslator *symbol_translator.SymbolTranslator, sugaredLogger *zap.SugaredLogger) (types.CosmosMsg, error) {
	...
	for _, attribute := range attributes {
		...
		case types.Symbol.String():
			...
			switch claimType {
			case types.MsgLock:
				symbol = symbolTranslator.GridironchainToEthereum(val)
			case types.MsgBurn:
				if !strings.Contains(val, defaultGridironchainPrefix) {
					// log.Printf("Can only relay burns of '%v' prefixed coins", defaultGridironchainPrefix)
					sugaredLogger.Errorw("only relay burns prefixed coins", "coin symbol", val)
					return types.CosmosMsg{}, errors.New("can only relay burns of '%v' prefixed coins" + defaultGridironchainPrefix)
				}
				res := strings.SplitAfter(val, defaultGridironchainPrefix)
