...
import (
	...
	clptest "github.com/Gridironchain/gridnode/x/clp/test"
	...
	for _, tt := range borrowTests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				err := gridapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(nativeCoin))
				require.Nil(t, err)
			} else {
				address = tt.address
			}
	...
	for _, tt := range interestRateComputationTests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			data := types.GenesisState{Params: &types.Params{
				...
				ForceCloseFundAddress:                    "did:fury:g1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd",
				IncrementalInterestPaymentFundPercentage: sdk.NewDecWithPrec(1, 1),
				IncrementalInterestPaymentFundAddress:    "did:fury:g1syavy2npfyt9tcncdtsdzf7kny9lh777yqc2nd",
...
func initKeeper(t testing.TB) (sdk.Context, *gridapp.GridironchainApp, types.Keeper) {
	ctx, app := test.CreateTestAppMargin(false)
	marginKeeper := app.MarginKeeper
	require.NotNil(t, marginKeeper)
	return ctx, app, marginKeeper
}
func addMTPKey(t testing.TB, ctx sdk.Context, app *gridapp.GridironchainApp, marginKeeper types.Keeper, collateralAsset string, custodyAsset string, address string, position types.Position, id uint64, health sdk.Dec) types.MTP {
