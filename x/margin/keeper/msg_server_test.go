	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				err = gridapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, _signer, sdk.NewCoins(nativeCoin, externalCoin))
				require.Nil(t, err)
			} else {
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fundedAccount {
				...
				_signer := clptest.GenerateAddress(clptest.AddressKey1)
				address = _signer.String()
				err = gridapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, _signer, sdk.NewCoins(nativeCoin))
				require.NoError(t, err)
...
func TestKeeper_OpenClose(t *testing.T) {
	...
	for _, tt := range table {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err = gridapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, signer, sdk.NewCoins(nativeCoin, externalCoin))
			require.Nil(t, err)
