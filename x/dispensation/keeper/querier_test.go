...
import (
	...
	dispensationkeeper "github.com/Gridironchain/gridnode/x/dispensation/keeper"
...
func TestNewLegacyQuerier(t *testing.T) {
	...
	keeper := gridapp.DispensationKeeper
	...
	qp, errRes := gridapp.LegacyAmino().MarshalJSON(&queryRecName)
	...
	err = gridapp.LegacyAmino().UnmarshalJSON(res, &dr)
	assert.Error(t, err)

}

func GenerateQueryData(app *app.GridironchainApp, ctx sdk.Context, name string, outList []bank.Output) {
...
func TestQueryRecordsName(t *testing.T) {
	...
	GenerateQueryData(gridapp, ctx, name, outList)
	keeper := gridapp.DispensationKeeper
	querier := dispensationkeeper.NewLegacyQuerier(keeper)
	queryRecName := types.QueryRecordsByDistributionNameRequest{
		DistributionName: name,
...
func TestQueryRecordsAddr(t *testing.T) {
	...
	qp, errRes := gridapp.LegacyAmino().MarshalJSON(&quereyRecName)
	require.NoError(t, errRes)
	query.Path = ""
	query.Data = qp
