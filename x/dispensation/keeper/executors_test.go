...
import (
	...
	"github.com/Gridironchain/gridnode/x/dispensation/utils"
...
func createInput(t *testing.T, filename string) {
	in, err := sdk.AccAddressFromBech32("did:fury:g1syavy2npfyt9tcncdtsdzf7kny9lh777vuvqz8")
...
func TestKeeper_AccumulateDrops(t *testing.T) {
	...
	for _, in := range inputList {
		...
		assert.NoError(t, err)
		err = gridapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, address, in.Coins)
		assert.NoError(t, err)
	}
}
...
func TestKeeper_DistributeDrops_Fail(t *testing.T) {
	...
	err = gridapp.AddCoinsToAccount(types.ModuleName, app.BankKeeper, ctx, dispensationCreator, totalCoins)
	assert.NoError(t, err)
