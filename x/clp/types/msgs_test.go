...
func GetFURYAsset() Asset {
	return NewAsset("fury")
}
...
func TestNewMsgCreatePool(t *testing.T) {
	...
	newpool = NewMsgCreatePool(signer, GetFURYAsset(), sdk.NewUint(1000), sdk.NewUint(100))
	err = newpool.ValidateBasic()
	assert.Error(t, err, "External Asset cannot be fury")
