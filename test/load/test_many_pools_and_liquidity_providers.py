...
# Example usage:
# cd test/load
# ../integration/framework/gridtool venv
...
#
# (4) Use parameter rpc.laddr for grided start instead of self.node
...
class Test:
    def __init__(self, cmd: command.Command, prj: project.Project, grided_home_root: str):
        self.cmd = cmd
        self.prj = prj
        ...
        self.grided_home_root = grided_home_root
    ...
    def setup(self):
        ...
        if self.wallets_dir is not None:
            ...
            client_home = os.path.join(self.grided_home_root, "grided-client")
        ...
        grided_client = gridironchain.Gridnoded(self.cmd, home=client_home)
        ...
        for i in range(self.number_of_wallets):
            ...
            if predefined_wallets:
                ...
                addr = grided_client.create_addr(mnemonic=mnemonic)
        ...
        grided_client = gridironchain.Gridnoded(self.cmd, home=client_home, node=gridironchain.format_node_url(
            self.env.node_info[0]["host"], self.env.node_info[0]["ports"]["rpc"]), chain_id=env.chain_id)
        grided_client.get_balance_default_retries = 5
        ...
        for addr, denoms in wallets.items():
            account_number, account_sequence = grided.get_acct_seq(addr)
            for denom in denoms:
                res = grided_client.tx_clp_add_liquidity(addr, denom, self.amount_of_liquidity_added_by_wallet,
                    self.amount_of_liquidity_added_by_wallet, account_seq=(account_number, account_sequence))
                gridironchain.check_raw_log(res)
                account_sequence += 1
        grided_client.wait_for_last_transaction_to_be_mined()
        self.check_actual_liquidity_providers(grided_client, env.clp_admin, wallets)

        self.grided = grided
        self.grided_client = grided_client

    def run(self):
        grided = self.grided
        grided_client = self.grided_client
        ...
        current_block = grided_client.get_current_block()
        ...
        if self.rewards_duration_blocks > 0:
            ...
            grided.clp_reward_period(admin_addr, reward_params)
            grided.wait_for_last_transaction_to_be_mined()
            wait_boundaries.add(rewards_start_block)
            wait_boundaries.add(rewards_end_block)
        # TODO grided query reward params --node --chain-id (check if/when implemented)
        ...
        if self.lpd_duration_blocks > 0:
            ...
            grided.clp_set_lppd_params(admin_addr, lppd_params)
            grided.wait_for_last_transaction_to_be_mined()
    ...
    # TODO Refactor - move to Gridnoded
    def wait_for_block(self, block_number: int) -> float:
        grided = self.grided
        current_block = grided.get_current_block()
        prev_block = None
            ...
            if self.block_results_offset is not None:
                if (prev_block is None) or (current_block != prev_block):
                    ...
                    try:
                        ...
                        blk = grided.get_block_results(height=height)
                        histogram = {}
    ...
    def check_actual_liquidity_providers(self, grided, clp_admin, wallets):
        actual_lp_providers = {}
        for denom in self.tokens:
            for lp in grided.query_clp_liquidity_providers(denom):
...
def run_test_case(args):
    ...
    grided_home_root = cmd.tmpdir("gridtool-test.tmp")
    cmd.rmdir(grided_home_root)
    test = Test(cmd, prj, grided_home_root=grided_home_root)
    ...
    try:
        ...
        try:
            ...
            balance = cosmos.balance_format(test.grided[0].get_balance(addr))
