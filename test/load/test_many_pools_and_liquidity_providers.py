...
#
# (4) Use parameter rpc.laddr for gridnoded start instead of self.node
...
class Test:
    def __init__(self, cmd: command.Command, prj: project.Project, gridnoded_home_root: str):
        self.cmd = cmd
        self.prj = prj
        ...
        self.gridnoded_home_root = gridnoded_home_root
    ...
    def setup(self):
        ...
        if self.wallets_dir is not None:
            ...
            client_home = os.path.join(self.gridnoded_home_root, "gridnoded-client")
        ...
        for i in range(self.number_of_wallets):
            ...
            if predefined_wallets:
                ...
                addr = gridnoded_client.create_addr(mnemonic=mnemonic)
        ...
        for addr, denoms in wallets.items():
            account_number, account_sequence = gridnoded.get_acct_seq(addr)
            for denom in denoms:
        ...
        self.gridnoded_client = gridnoded_client

    def run(self):
        gridnoded = self.gridnoded
        gridnoded_client = self.gridnoded_client
        ...
        current_block = gridnoded_client.get_current_block()
        ...
        # TODO gridnoded query reward params --node --chain-id (check if/when implemented)
    ...
    def wait_for_block(self, block_number: int) -> float:
        gridnoded = self.gridnoded
        current_block = gridnoded.get_current_block()
        prev_block = None
            ...
            if self.block_results_offset is not None:
                if (prev_block is None) or (current_block != prev_block):
                    ...
                    try:
                        ...
                        blk = gridnoded.get_block_results(height=height)
                        histogram = {}
    ...
    def check_actual_liquidity_providers(self, gridnoded, clp_admin, wallets):
        actual_lp_providers = {}
        for denom in self.tokens:
            for lp in gridnoded.query_clp_liquidity_providers(denom):
...
def run_test_case(args):
    ...
    gridnoded_home_root = cmd.tmpdir("gridtool-test.tmp")
    cmd.rmdir(gridnoded_home_root)
    test = Test(cmd, prj, gridnoded_home_root=gridnoded_home_root)
    ...
    try:
        ...
        try:
            ...
            balance = cosmos.balance_format(test.gridnoded[0].get_balance(addr))
