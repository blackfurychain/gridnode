...
gridnode_funds_for_transfer_peggy1 = 10**17  # fury
...
def get_env_ctx(cmd=None, env_file=None, env_vars=None):
    ...
    # Add any Gridironchain private keys to test keystore
    grid_user_private_keys = ctx.cmd.project.read_peruser_config_file("grid-keys")
    if grid_user_private_keys:
        available_grid_accounts = ctx.gridnode.keys_list()
        for name, address, mnemonic in [[e["name"], e["address"], e["mnemonic"].split(" ")] for e in grid_user_private_keys]:
            existing_acct = [a for a in available_grid_accounts if a["address"] == address]
            if not existing_acct:
                acct = ctx.gridnode.keys_add(name, mnemonic)
                assert acct["address"] == address, "Invalid address for grid account {}".format(name)
    return ctx

def get_env_ctx_peggy2():
    cmd = run_env.Integrator()

    if "GRIDTOOL_ENV_FILE" in os.environ:
        # New-style format (hopefully unified)
        # This is for connecting to peggy2-tempnet etc.
        env_file = os.environ["GRIDTOOL_ENV_FILE"]
        ...
        # For either `gridtool run-env` or `devenv`
...
def get_env_ctx_peggy1(cmd=None, env_file=None, env_vars=None):
    ...
    if "CHAINNET" in env_vars:
        gridnode_chain_id = env_vars["CHAINNET"]
    elif deployment_name:
        gridnode_chain_id = deployment_name
    else:
        gridnode_chain_id = "localnet"
    ...
    generic_erc20_contract_name = "GridironchainTestToken"
...
def grid_addr_to_evm_arg(grid_address):
    return grid_address.encode("UTF-8")
...
class EnvCtx:
    ...
    def tx_deploy_new_generic_erc20_token(self, deployer_addr: str, name: str, symbol: str, decimals: int, cosmosDenom: str = None) -> Contract:
        # return self.tx_deploy("GridironchainTestToken", self.operator, [name, symbol, decimals])
        if on_peggy2_branch:
            # Use BridgeToken
            assert self.generic_erc20_contract == "BridgeToken"
            if cosmosDenom is None:
                cosmosDenom = "erc20denom"  # TODO Dummy variable since we're using BridgeToken instead of GridironchainTestToken

            constructor_args = [name, symbol, decimals, cosmosDenom]
        else:
            # Use GridironchainTestToken for TestNet and Devnet, and BridgeToken for Betanet
    ...
    def tx_bridge_bank_lock_eth(self, from_eth_acct, to_grid_acct, amount):
        recipient = grid_addr_to_evm_arg(to_grid_acct)
        bridge_bank = self.get_bridge_bank_sc()
    ...
    def tx_bridge_bank_lock_erc20(self, token_addr, from_eth_acct, to_grid_acct, amount):
        recipient = grid_addr_to_evm_arg(to_grid_acct)
        bridge_bank = self.get_bridge_bank_sc()
    ...
    def tx_bridge_bank_burn_erc20(self, token_addr: str, from_eth_acct: str, to_grid_acct: str, amount: int) -> HexBytes:
        recipient = grid_addr_to_evm_arg(to_grid_acct)
        bridge_bank = self.get_bridge_bank_sc()
        # When transfering ERC20, the amount needs to be passed as argument, and the "message.value" should be 0
        tx_opts = {"value": 0}
    ...
    def tx_approve_and_lock(self, token_sc, from_eth_acct, to_grid_acct, amount):
        bridge_bank_sc = self.get_bridge_bank_sc()
        txhash1 = self.tx_approve(token_sc, self.operator, bridge_bank_sc.address, amount)
        txhash2 = self.tx_bridge_bank_lock_erc20(token_sc.address, from_eth_acct, to_grid_acct, amount)
        log.debug("tx_approve_and_lock: {} '{}' ({}) from {} to {}".format(amount, token_sc.functions.name().call(),
            token_sc.functions.symbol().call(), from_eth_acct, to_grid_acct))
    ...
    def create_new_currency(self, symbol, name, decimals, amount, minted_tokens_recipient):
        ...
        1. Deploys a new instance of GridironchainTestToken
        ...
        """
        assert self.generic_erc20_contract == "GridironchainTestToken"  # Preserve compatibiliy with integration tests and inflate_tokens.sh
    ...
    def create_gridironchain_addr(self, moniker: str = None,
        ...
        acct = self.gridnode.keys_add(moniker)
    ...
    def get_current_block(self):
        return self.gridnode.get_current_block()

    # TODO Clean up
    def status(self):
        return self.gridnode.status()
    ...
    def bridge_bank_lock_eth(self, from_eth_acct, to_grid_acct, amount):
        """ Sends ETH from Ethereum to Gridironchain (lock) """
        txhash = self.tx_bridge_bank_lock_eth(from_eth_acct, to_grid_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)

    def bridge_bank_lock_erc20(self, token_sc, from_eth_acct, to_grid_acct, amount):
        txhash = self.tx_bridge_bank_lock_erc20(token_sc.address, from_eth_acct, to_grid_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)

    def bridge_bank_burn_erc20(self, token_sc: Contract, from_eth_acct: str, to_grid_acct: str, amount: int) -> TxReceipt:
        txhash = self.tx_bridge_bank_burn_erc20(token_sc.address, from_eth_acct, to_grid_acct, amount)
        return self.eth.wait_for_transaction_receipt(txhash)
    ...
    def send_from_ethereum_to_gridironchain(self, from_eth_acct: str, to_grid_acct: str, amount: int, token_sc: Contract = None, isLock: bool = True) -> TxReceipt:
        if token_sc is None:
            ...
            if isLock:
                self.bridge_bank_lock_erc20(token_sc, from_eth_acct, to_grid_acct, amount)
            else:
                self.bridge_bank_burn_erc20(token_sc, from_eth_acct, to_grid_acct, amount)
    ...
    def sanity_check(self):
        ...
        available_accounts = self.gridnode.keys_list()
