...
from gridtool import command, cosmos, eth
...
# Default ports
GRIDNODED_DEFAULT_RPC_PORT = 26657  # In config/config.toml; used for --node, can be queried using curl http://...
GRIDNODED_DEFAULT_P2P_PORT = 26656  # In config/config.toml
GRIDNODED_DEFAULT_API_PORT = 1317  # In config/app.toml, section [api], disabled by default
GRIDNODED_DEFAULT_GRPC_PORT = 9090  # In config/app.toml, section [grpc]
...
# TODO This should be dynamic (per-Gridnoded)
grid_tx_burn_fee_in_fury = 100000
grid_tx_burn_fee_in_ceth = 1

# TODO This should be dynamic (per-Gridnoded)
# There seems to be a minimum amount of fury that a grid account needs to own in order for the bridge to do an
...
# Fee for transfering ERC20 tokens from an ethereum account to grid account (approve + lock). This is the maximum cost
...
def is_cosmos_native_denom(denom: str) -> bool:
    ...
    if on_peggy2_branch:
        return not str.startswith(denom, "gridBridge")
    else:
...
def mnemonic_to_address(cmd: command.Command, mnemonic: Iterable[str]):
    ...
    gridnode = Gridnoded(cmd, home=tmpdir)
    try:
       return gridnode.keys_add("tmp", mnemonic)["address"]
    finally:
        cmd.rmdir(tmpdir)
...
def check_raw_log(res: JsonDict):
    ...
    last_line = lines[-1]
    raise GridnodedException(last_line)
...
class Gridnoded:
    ...
    def enable_rpc_port(self):
        ...
        app_toml["api"]["address"] = format_node_url(ANY_ADDR, GRIDNODED_DEFAULT_API_PORT)
    ...
    # from_grid_addr has to be the address which was used at genesis time for "set-genesis-whitelister-admin".
    ...
    def token_registry_register(self, entry: TokenRegistryParams, from_grid_addr: cosmos.Address,
        ...
        assert self.keys_show(from_grid_addr)
        ...
        with self._with_temp_json_file(token_data) as tmp_registry_json:
            args = ["tx", "tokenregistry", "register", tmp_registry_json, "--from", from_grid_addr, "--output",
    ...
    def token_registry_register_batch(self, from_grid_addr: cosmos.Address, entries: Iterable[TokenRegistryParams]):
        account_number, account_sequence = self.get_acct_seq(from_grid_addr)
        token_registry_entries_before = set(e["denom"] for e in self.query_tokenregistry_entries())
        for entry in entries:
            res = self.token_registry_register(entry, from_grid_addr, account_seq=(account_number, account_sequence))
            check_raw_log(res)
            account_sequence += 1
    ...
    def build_start_cmd(self, tcp_url: Optional[str] = None, p2p_laddr: Optional[str] = None,
        ...
        args = [self.binary, "start"] + \
            (["--trace"] if trace else []) + \
            (["--minimum-gas-prices", grid_format_amount(*minimum_gas_prices)] if minimum_gas_prices is not None else []) + \
    ...
    def send(self, from_grid_addr: cosmos.Address, to_grid_addr: cosmos.Address, amounts: cosmos.Balance,
        account_seq: Optional[Tuple[int, int]] = None, broadcast_mode: Optional[str] = None
    ) -> JsonDict:
        ...
        args = ["tx", "bank", "send", from_grid_addr, to_grid_addr, amounts_string, "--output", "json"] + \
    ...
    def send_and_check(self, from_addr: cosmos.Address, to_addr: cosmos.Address, amounts: cosmos.Balance
        ...
        expected_tx_fee = {FURY: grid_tx_fee_in_fury}
    ...
    def get_balance(self, grid_addr: cosmos.Address, height: Optional[int] = None,
        disable_log: bool = False, retries_on_error: Optional[int] = None, delay_on_error: int = 3
    ) -> cosmos.Balance:
        base_args = ["query", "bank", "balances", grid_addr]
    ...
    def wait_for_balance_change(self, grid_addr: cosmos.Address, old_balance: cosmos.Balance,
        ...
        log.debug("Waiting for balance to change for account {}...".format(grid_addr))
        ...
        while True:
            new_balance = self.get_balance(grid_addr, disable_log=disable_log)
            ...
            if (timeout is not None) and (timeout > 0) and (now - start_time > timeout):
                raise GridnodedException("Timeout waiting for grid account {} balance to change ({}s)".format(grid_addr, timeout))
            if last_change_time is None:
                ...
                if (change_timeout is not None) and (change_timeout > 0) and (now - last_change_time > change_timeout):
                    raise GridnodedException("Timeout waiting for grid account {} balance to change ({}s)".format(grid_addr, change_timeout))
    ...
    # See https://github.com/Gridironchain/gridnode/blob/master/docs/tutorials/clp%20tutorial.md
    ...
    def sign_transaction(self, tx: JsonDict, from_grid_addr: cosmos.Address, sequence: int = None,
    ...
    def _get_host_and_port(self) -> Tuple[str, int]:
        ...
        if self.node is None:
            return LOCALHOST, GRIDNODED_DEFAULT_RPC_PORT
        else:
    ...
    def wait_for_last_transaction_to_be_mined(self, count: int = 1, disable_log: bool = True, timeout: int = 90):
        log.debug("Waiting for last gridnode transaction to be mined...")
        start_time = time.time()
        initial_block = self.get_current_block()
        while self.get_current_block() < initial_block + count:
            time.sleep(1)
            if time.time() - start_time > timeout:
                raise Exception("Timeout expired while waiting for last gridnode transaction to be mined")
...
# Refactoring in progress - this class is supposed to become the successor of class Gridnode.
...
class GridnodeClient:
    def __init__(self, ctx, gridnode: Gridnoded, node: Optional[str] = None, chain_id: Optional[str] = None,
    ...
    def send_from_gridironchain_to_ethereum(self, from_grid_addr: cosmos.Address, to_eth_addr: str, amount: int, denom: str,
        ...
        if on_peggy2_branch:
            ...
            args = ["tx", "ethbridge", direction, to_eth_addr, str(amount), denom, str(cross_chain_ceth_fee),
                    ...
                    "--from", from_grid_addr,  # Mandatory, either name from keyring or address
                    "--output", "json",
                ] + \
                (["--generate-only"] if generate_only else []) + \
                self.gridnode._fees_args() + \
                self.gridnode._home_args() + \
                (self.gridnode._keyring_backend_args() if not generate_only else []) + \
            ...
            args = ["tx", "ethbridge", direction] + \
                self.gridnode._node_args() + \
                [from_grid_addr, to_eth_addr, str(amount), denom, cross_chain_ceth_fee] + \
                (self.gridnode._keyring_backend_args() if not generate_only else []) + \
                self.gridnode._fees_args() + \
...
class Gridgen:
    ...
    # Reference: docker/localnet/gridnode/root/scripts/gridnode.sh (branch future/peggy2):
    ...
    def create_standalone(self, chainnet, moniker, mnemonic, bind_ip_address, keyring_backend=None):
        ...
        return self.gridgen_exec(args, keyring_backend=keyring_backend)

    def gridgen_exec(self, args, keyring_backend=None, cwd=None, env=None):
        args = [self.binary] + args + \
...
class Ebrelayer:
    ...
    def init(self, tendermind_node, web3_provider, bridge_registry_contract_address, validator_moniker,
        ...
        args = [self.binary, "init", tendermind_node, web3_provider, bridge_registry_contract_address,
            ...
            (["--gas-prices", grid_format_amount(*gas_prices)] if gas_prices is not None else []) + \
...
class GridnodedException(Exception):
...
def is_min_commission_too_low_exception(e: Exception):
    ...
    return (type(e) == GridnodedException) and patt.match(e.message)
