...
import gridtool.eth
from gridtool import eth, cosmos, command
from gridtool.hardhat import Hardhat, default_accounts as hardhat_default_accounts
from gridtool.geth import Geth
...
class Integrator(Ganache, Command):
    ...
    def grided_peggy2_init_validator(self, gridnode, validator_moniker, validator_mnemonic, evm_network_descriptor, validator_power):
        ...
        valoper = gridnode.get_val_address(validator_moniker)

        # Add genesis validator
        gridnode.add_genesis_validators_peggy(evm_network_descriptor, valoper, validator_power)

    # @TODO Move to Gridgen class
    def gridgen_create_network(self, chain_id: str, validator_count: int, networks_dir: str, network_definition_file: str,
        seed_ip_address: str, mint_amount: Optional[cosmos.Balance] = None
    ):
        # Old call (no longer works either):
        # gridgen network create localnet 1 /mnt/shared/work/projects/grid/gridnode/local-tmp/my/deploy/rake/../networks \
        #     192.168.1.2 /mnt/shared/work/projects/grid/gridnode/local-tmp/my/deploy/rake/../networks/network-definition.yml \
        #     --keyring-backend file
        # self.cmd.execst(["gridgen", "network", "create", "localnet", str(validator_count), networks_dir, seed_ip_address,
        ...
        args = ["gridgen", "network", "create", chain_id, str(validator_count), networks_dir, seed_ip_address,
            network_definition_file, "--keyring-backend", "test"] + \
            (["--mint-amount", cosmos.balance_format(mint_amount)] if mint_amount else [])
        self.execst(args)

    # def wait_for_grid_account(self, netdef_json, validator1_address):
    #     # TODO Replace with test_utilities.wait_for_grid_account / wait_for_grid_account_up
    #     return self.execst(["python3", os.path.join(self.project.test_integration_dir, "src/py/wait_for_grid_account.py"),
    #         netdef_json, validator1_address], env={"USER1ADDR": "nothing"})

    def wait_for_grid_account_up(self, address: cosmos.Address, tcp_url: str = None, timeout: int = 90):
        # TODO Move to Gridnoded class
        ...
        #     time.sleep(1)
        # self.wait_for_grid_account(netdef_json, validator1_address)
        ...
        while True:
            try:
                ...
                if last_exception is None:
                    log.debug(f"Waiting for grid account {address}...")
                if time.time() - start_time > timeout:
                    message = "Timeout expired while waiting for grid account {} to become available".format(address)
...
class UIStackEnvironment:
    ...
    def stack_save_snapshot(self):
        ...
        assert shadowfiend_address == self.gridnode.keys_show(self.shadowfiend_name)[0]["address"]
        assert akasha_address == self.gridnode.keys_show(self.akasha_name)[0]["address"]
        assert juniper_address == self.gridnode.keys_show(self.juniper_name)[0]["address"]
        ...
        gridnode.add_genesis_account(shadowfiend_address, tokens_shadowfiend)
        gridnode.add_genesis_account(akasha_address, tokens_akasha)
        ...
        # ui/scripts/_migrate.sh -> ui/chains/grid/migrate.sh
        # Original scripts say "if we don't sleep there are issues"
        time.sleep(10)
        log.info("Creating liquidity pool from catk:fury...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "catk", [10**5, FURY], 10**25, 10**25)
        time.sleep(5)
        log.info("Creating liquidity pool from cbtk:fury...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "cbtk", [10**5, FURY], 10**25, 10**25)
        # should now be able to swap from catk:cbtk
        time.sleep(5)
        log.info("Creating liquidity pool from ceth:fury...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "ceth", [10**5, FURY], 10**25, 83*10**20)
        # should now be able to swap from x:ceth
        time.sleep(5)
        log.info("Creating liquidity pool from cusdc:fury...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "cusdc", [10**5, FURY], 10**25, 10**25)
        time.sleep(5)
        log.info("Creating liquidity pool from clink:fury...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "clink", [10**5, FURY], 10**25, 588235*10**18)
        time.sleep(5)
        log.info("Creating liquidity pool from ctest:fury...")
        gridnode.tx_clp_create_pool(self.chain_id, self.keyring_backend, "akasha", "ctest", [10**5, FURY], 10**25, 10**13)
...
class IntegrationTestsEnvironment:
    def __init__(self, cmd):
        ...
        # set_persistant_env_var GRIDCHAIN_BIN $BASEDIR/cmd $envexportfile
        ...
        # set_persistant_env_var CONTAINER_NAME integration_gridnode1_1 $envexportfile
        ...
        self.network_name = "develop"
        self.network_id = 5777
        self.peruser_storage_dir = self.cmd.get_user_home(".gridnode-integration")
        self.state_vars = {}
    ...
    def run(self):
        ...
        # New:
        # gridgen network create localnet 1 $NETWORKDIR 192.168.1.2 $NETWORKDIR/network-definition.yml --keyring-backend test \
        ...
        self.cmd.gridgen_create_network(chain_id, validator_count, networks_dir, network_definition_file, seed_ip_address, mint_amount=self.mint_amount)
...
class Peggy2Environment(IntegrationTestsEnvironment):
    def __init__(self, cmd: Command, witness_count: int = 1, witness_power: int = 100, consensus_threshold: int = 49,
        ...
        self.validator_mint_amounts: cosmos.Balance = {
            ...
            "gridBridge00030x1111111111111111111111111111111111111111": 137 * 10**16,
            "ibc/FEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACEFEEDFACE": 137 * 10**16,
        }
        ...
        self.admin_account_mint_amounts: cosmos.Balance = {
            ...
            "gridBridge00030x1111111111111111111111111111111111111111": 10 ** 16,
    ...
    def run(self) -> Tuple[subprocess.Popen, subprocess.Popen, subprocess.Popen, List[subprocess.Popen]]:
        ...
        admin_account_name = "gridnodeadmin"
        ...
        log.debug("Validator 0 address: {}".format(gridnode_validators[0]["address"]))  # mint
        ...
        [relayer0_exec_args], [witness_exec_args] = self.start_witnesses_and_relayers(
            ...
            gridnode_validators, gridnode_relayers, gridnode_witnesses, symbol_translator_file, self.log_level_relayer,
            self.log_level_witness, relayer_extra_args)

        hardhat_scripts.update_validator_power(peggy_sc_addrs["CosmosBridge"], evm_validator_addresses, gridnode_witnesses)
        ...
        _unused_peggy2_environment = {
            "admin": {
                ...
                "home": gridnode_validator0_home,
            },
            "symbol_translator_file": symbol_translator_file,
            "w3_url": w3_url,
            "evm_chain_id": self.ethereum_chain_id,
            "chain_id": self.chain_id,
            "validators": gridnode_validators,  # From yaml file generated by gridgen
            "relayers": gridnode_relayers,
    ...
    def init_gridironchain(self, grided_network_dir: str, grided_log_file: TextIO,
        ...
        gridnode = Gridnoded(self.cmd, home=validator0_home, chain_id=self.chain_id)

        # Create an ADMIN account on gridnode with name admin_account_name (e.g. "gridnodeadmin")
        admin_account_address = gridnode.peggy2_add_account(admin_account_name, admin_account_mint_amounts, is_admin=True)
        ...
        res = gridnode.peggy2_token_registry_set_registry(registry_json, gas_prices, 1.5, admin_account_address,
            self.chain_id)
        ...
        gas_prices = [0.5, FURY]
        gas_adjustment = 1.5
        gridnode.peggy2_set_cross_chain_fee(admin_account_address, self.ethereum_chain_id,
    ...
    def start_witnesses_and_relayers(self, web3_websocket_address: str, hardhat_chain_id: int, tcp_url: str,
        ...
        gridnode_validators: Sequence[Mapping[str, Any]], gridnode_relayers: Sequence[Mapping[str, Any]],
        gridnode_witnesses: Sequence[Mapping[str, Any]], symbol_translator_file: str, log_level_relayer: Optional[str],
        ...
        gridnode_validator0 = exactly_one(gridnode_validators)
        gridnode_validator0_address = gridnode_validator0["address"]

        gridnode_relayer0 = exactly_one(gridnode_relayers)
        gridnode_relayer0_mnemonic = gridnode_relayer0["name"]
        gridnode_relayer0_address = gridnode_relayer0["address"]
        gridnode_relayer0_home = gridnode_relayer0["home"]

        bridge_registry_contract_addr = peggy_sc_addrs["BridgeRegistry"]

        self.cmd.wait_for_grid_account_up(gridnode_validator0_address, tcp_url=tcp_url)  # Required for both relayer and witness
        ...
        relayer0_exec_args = ebrelayer.peggy2_build_ebrelayer_cmd(
            ...
            gridnode_relayer0_mnemonic,
            chain_id=chain_id,
            node=tcp_url,
            sign_with=gridnode_relayer0_address,
            symbol_translator_file=symbol_translator_file,
            ethereum_address=evm_validator0_addr,
            ethereum_private_key=evm_validator0_key,
            keyring_backend="test",
            keyring_dir=gridnode_relayer0_home,
            home=gridnode_relayer0_home,
            log_level=log_level_relayer,
            **relayer_extra_args,
        )

        witness_exec_args = []
        for i, w in enumerate(gridnode_witnesses):
    ...
    def write_env_files(self, project_dir, go_bin_dir, evm_smart_contract_addrs, eth_accounts, admin_account_name,
        ...
        def format_grid_account(grid_account):
            return {"account": grid_account["address"], "name": grid_account["name"], "homeDir": grid_account["home"]}
        ...
        environment_json = {
            ...
            "gridResults": {
                "validatorValues": gridnode_validators,
                "adminAddress": format_grid_account({"address": admin_account_address, "name": admin_account_name, "home": gridnode_validator0_home}),
                "relayerAddresses": [format_grid_account(a) for a in gridnode_relayers],
                "witnessAddresses": [format_grid_account(a) for a in gridnode_witnesses],
                # "process": { ... },
                "tcpurl": tcp_url,
            }
        }
        ...
        dot_env = dict_merge({
            ...
            "GOBIN": go_bin_dir,
            "TCP_URL": tcp_url,
            "VALIDATOR_ADDRESS": gridnode_validators[0]["address"],
            "VALIDATOR_CONSENSUS_ADDRESS": gridnode_validators[0]["validator_consensus_address"],
            "VALIDATOR_MENOMONIC": gridnode_validators[0]["mnemonic"],
            "VALIDATOR_MONIKER": gridnode_validators[0]["moniker"],
            "VALIDATOR_PASSWORD": gridnode_validators[0]["password"],
            "VALIDATOR_PUB_KEY": gridnode_validators[0]["pub_key"],
        ...
        } for i, validator in enumerate(gridnode_validators)], *[{
            f"RELAYER_ADDRESS_{i}": relayer["address"],
            f"RELAYER_NAME_{i}": relayer["name"],
        } for i, relayer in enumerate(gridnode_relayers)], *[{
            f"WITNESS_ADDRESS_{i}": witness["address"],
            f"WITNESS_NAME_{i}": witness["name"],
        } for i, witness in enumerate(gridnode_witnesses)])
        ...
        launch_json = {
            ...
            "configurations": [
                {
                    "name": "Python: gridtool",
                    "type": "python",
                    "request": "launch",
                    "program": "${workspaceFolder}/test/integration/framework/gridtool",
                    "args": ["run-env"],
        ...
        for config in launch_json["configurations"]:
            if config["name"].startswith("Debug Relayer"):
                intellij_ebrelayer_configs.append(render_intellij_run_xml(
                    "ebrelayer devenv",
                    " ".join(config["args"]),
                    "github.com/Gridironchain/gridnode/cmd/ebrelayer",
...
class IBCEnvironment(IntegrationTestsEnvironment):
    ...
    def run(self):
        ...
        gridgen = Gridgen(self.cmd)
        # This does not work - "--keyring-backend" is not supported
        x = gridgen.create_standalone(chainnet0, "chain1", mnemonic, ipaddr0, keyring_backend=None)
