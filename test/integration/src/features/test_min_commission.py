...
def create_environment(cmd, version, commission_rate=0.06, commission_max_rate=0.10, commission_max_change_rate=0.05,
    ...
    home_root = "/tmp/gridtool.tmp/test_min_commission"
    cmd.rmdir(home_root)
    cmd.mkdir(home_root)
...
def test_max_voting_power(cmd: command.Command):
    ...
    test_case(akasha, grid, 100, False)  # Current (and projected) voting power too big
    test_case(grid, akasha, 100, True)
    test_case(grid, akasha, 10**23, False)  # Projected voting power too big
