...
@dataclass
class GridironchaincliCredentials:
...
def get_eth_balance(transfer_request: EthereumToGridironchainTransferRequest):
...
def get_whitelisted_tokens(transfer_request: EthereumToGridironchainTransferRequest):
    network_element = f"--ethereum_network {transfer_request.ethereum_network} " if transfer_request.ethereum_network else ""
    symbol_element = f"--symbol {transfer_request.ethereum_symbol} " if transfer_request.ethereum_symbol else ""
    private_element = f"--ethereum_private_key_env_var \"{transfer_request.ethereum_private_key_env_var}\"" if transfer_request.ethereum_private_key_env_var else ""
    command_line = " ".join(
...
def mint_tokens(transfer_request: EthereumToGridironchainTransferRequest, operator_address):
    network_element = f"--ethereum_network {transfer_request.ethereum_network} " if transfer_request.ethereum_network else ""
    symbol_element = f"--symbol {transfer_request.ethereum_symbol} " if transfer_request.ethereum_symbol else ""
    private_element = f"--ethereum_private_key_env_var \"{transfer_request.ethereum_private_key_env_var}\"" if transfer_request.ethereum_private_key_env_var else ""
    command_line = " ".join(
...
def wait_for_eth_balance(transfer_request: EthereumToGridironchainTransferRequest, target_balance, max_seconds=80):
    wait_for_balance(
...
def send_from_gridironchain_to_ethereum_cmd(
    ...
    """
    Sends from Gridironchain to Ethereum.
...
def wait_for_grid_account(grid_addr, gridironchaincli_node, max_seconds=90):
    ...
    wait_for_predicate(lambda: fn(), True, max_seconds, f"wait for account {grid_addr}")
...
def wait_for_ethereum_block_number(block_number: int, transfer_request: EthereumToGridironchainTransferRequest):
    network_element = f"--ethereum_network {transfer_request.ethereum_network} " if transfer_request.ethereum_network else ""
...
def approve_token_amount(token_request: EthereumToGridironchainTransferRequest):
