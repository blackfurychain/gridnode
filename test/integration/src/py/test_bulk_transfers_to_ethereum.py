...
def test_bulk_transfers_from_gridchain(
    ...
    logging.info("create new ethereum and gridchain addresses")
    ...
    for a in range(n_transfers):
        for t in tokens:
            request.gridchain_destination_address, from_key = addresses_to_populate.pop()

            # send ceth to pay for the burn
            request.amount = test_utilities.burn_gas_cost
            request.gridchain_symbol = "ceth"
            burn_lock_functions.transfer_gridchain_to_gridchain(request, credentials_for_account_with_ceth)

            # send fury to pay the fee
            request.amount = gridchain_fees_int
            request.gridchain_symbol = "fury"
            burn_lock_functions.transfer_gridchain_to_gridchain(request, credentials_for_account_with_ceth)

            # send the token itself
            request.amount = test_transfer_amount
            request.gridchain_symbol = t
            burn_lock_functions.transfer_gridchain_to_gridchain(request, credentials_for_account_with_ceth)
            transfer = (request.gridchain_destination_address, from_key, request.gridchain_symbol, request.amount)

            test_utilities.get_gridchain_addr_balance(request.gridchain_destination_address, request.gridnoded_node, t)
    ...
    for gridaddr, from_key, gridsymbol, amount in test_transfers:
        ...
        r.gridchain_symbol = gridsymbol
        r.gridchain_address = gridaddr
        ...
        ethereum_symbol = test_utilities.gridchain_symbol_to_ethereum_symbol(gridsymbol)
