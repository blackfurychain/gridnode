...
from test_utilities import GridironchaincliCredentials
...
def test_rescue_ceth(
    ...
    admin_user_credentials = GridironchaincliCredentials(
    ...
    test_account_request, test_account_credentials = generate_test_account(
        ...
        target_fury_balance=gridironchain_fees_int
    )
    logging.info("get the starting balance for the ethbridge module - that's where fees should be going")
    ethbridge_module_balance = test_utilities.get_gridironchain_addr_balance(
        ethbridge_module_address,
        basic_transfer_request.grided_node,
        "ceth"
    )
    test_account_request.amount = small_amount
    burn_lock_functions.transfer_gridironchain_to_ethereum(test_account_request, test_account_credentials)
    logging.info(
        f"test account {test_account_request.gridironchain_address} should now have no ceth")
    logging.info("ethbridge should have the fee that was paid")
    test_utilities.wait_for_gridironchain_addr_balance(
        ethbridge_module_address,
        "ceth",
        ethbridge_module_balance + test_utilities.burn_gas_cost,
        test_account_request.grided_node
    )
    logging.info(f"rescue ceth into {test_account_request.gridironchain_address}")
    test_utilities.rescue_ceth(
        receiver_account=test_account_request.gridironchain_address,
    ...
    test_utilities.wait_for_gridironchain_addr_balance(
        test_account_request.gridironchain_address,
        "ceth",
...
def test_ceth_receiver_account(
    ...
    basic_transfer_request.gridironchain_address = validator_address
    admin_user_credentials = gridironchain_admin_account_credentials
    ...
    received_ceth_charges = test_utilities.get_gridironchain_addr_balance(ceth_rescue_account,
                                                                     basic_transfer_request.grided_node, "ceth")
...
def test_fee_charged_to_transfer_fury_to_efury(
    ...
    starting_ceth_balance = test_utilities.get_gridironchain_addr_balance(request.gridironchain_address, request.grided_node,
                                                                     "ceth")
    burn_lock_functions.transfer_gridironchain_to_ethereum(request, credentials)
    ending_ceth_balance = test_utilities.get_gridironchain_addr_balance(request.gridironchain_address, request.grided_node,
                                                                   "ceth")
