...
```
make griddocker  # builds the docker image
make griddocker-start  # starts the docker container and leaves it running
make griddocker-sh     # gives you a shell in the running container
```
## Execute

[start-integration-env.sh](./start-integration-env.sh) starts 
gridnoded and ganache.

Run the tests in a container with:

```
cd /gridnode/test/integration
...
```
# cd ~/workspace/gridnode/test/integration 
# source ./exampleenv.sh
# gridnoded q auth account --node tcp://44.241.55.154:26657 grid1pvnu2kh826vn8r0ttlgt82hsmfknvcnf7qmpvk
# ...
```
...
This creates a new gridchain account with 120000000000 tokens of each of the tokens in the whitelist.  
...
where FURY_SOURCE is a gridnode address with sufficient tokens to distribute.
