...
$(grided keys show user2 -a) $(grided keys show user1 -a --bech val) 10000 lock \
...
grided q auth account $(grided keys show user2 -a)
...
grided tx ethbridge burn $(grided keys show user2 -a) 0x11111111262b236c9ac9a9a8c8e4276b5cf6b2c9 \
...
grided tx ethbridge lock $(grided keys show user2 -a) 0x11111111262b236c9ac9a9a8c8e4276b5cf6b2c9 \
...
$(grided keys show user2 -a) $(grided keys show user1 -a --bech val) \
...
sleep 8
yes Y | grided tx clp create-pool --from user2 --symbol ceth --nativeAmount 200 --externalAmount 200
sleep 8
yes Y | grided tx clp create-pool --from user2 --symbol cdash --nativeAmount 100 --externalAmount 100
...
sleep 8
grided query clp lp ceth $(grided keys show user2 -a)

echo "adding more liquidity"
sleep 8
yes Y | grided tx clp add-liquidity --from user2 --symbol ceth --nativeAmount 1 --externalAmount 1

echo "swap"
sleep 8
yes Y |  grided tx clp swap --from user2 --sentSymbol ceth --receivedSymbol cdash --sentAmount 20 ---minReceivingAmount 0


echo "removing Liquidity"
sleep 8
yes Y | grided tx clp remove-liquidity --from user2 --symbol ceth --wBasis 5001 --asymmetry -1
...
sleep 8
yes Y | grided tx clp decommission-pool --from user2 --symbol ceth

echo "grided query clp pools -> should list both pools / Decommission can only be done by admin users"