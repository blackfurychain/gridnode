...
Gridironchain is switching from a slip-based fee model to a fixed-rate fee model. This requires
the swap formula and the liquidity fee formula within gridnode to be updated.
...
```bash
gridnoded q clp swap-fee-rate --output json
```
