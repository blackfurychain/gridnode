...
const gridnodeDestinationFile = generateTodayFilename({
  directory: "data",
  prefix: "gridnode_mainnet_update",
  extension: "json",
});
...
async function main() {
  ...
  for (let i = 0; i < addressList.length; i++) {
    try {
      ...
      // Now, the gridnode side:
      const gridnodeObj = { ...GRIDNODE_MODEL };
      const v1Denom = generateV1Denom(symbol);
      gridnodeObj.decimals = decimals;
      gridnodeObj.base_denom = v1Denom;
      gridnodeObj.denom = v1Denom;
      gridnodeList.push(gridnodeObj);
  ...
  fs.writeFileSync(
    gridnodeDestinationFile,
    JSON.stringify(gridnodeList, null, 2)
  );
  ...
  print(
    "magenta",
    `Gridnode results have been written to ${gridnodeDestinationFile}.`
  );
  print(
    "yellow",
