...
contract("BridgeBank", function (accounts) {
  ...
  describe("Bridge token minting (for burned Cosmos assets)", function () {
    ...
    it("should return false if a gridironchain address has an incorrect `grid` prefix", async function () {
      ...
      (await this.bridgeBank.verifyGridPrefix(incorrectGridAddress)).should.be.equal(false);
    })
  ...
  describe("Bridge token deposit locking (Ethereum/ERC20 assets)", function () {
    ...
    it("should not allow users to lock ERC20 tokens if the gridaddress length is incorrect", async function () {
      const invalidGridAddress = web3.utils.utf8ToHex(
    ...
    it("should not allow users to lock ERC20 tokens if the gridaddress prefix is incorrect", async function () {
      const invalidGridAddress = web3.utils.utf8ToHex(
