...
const eFury = artifacts.require("BridgeToken");
...
module.exports = function(deployer, network, accounts) {
  ...
  deployer.then(async () => {
    ...
    const efury = await deployer.deploy(eFury, "efury", setTxSpecifications(3000000, operator));

    await efury.addMinter(BridgeBank.address, setTxSpecifications(3000000, operator));

    await bridgeBank.addExistingBridgeToken(efury.address, setTxSpecifications(3000000, operator));

    const tokenAddress = "0x0000000000000000000000000000000000000000";

    // allow 10 eth to be sent at once
    await efury.approve(bridgeBank.address, '10000000000000000000', setTxSpecifications(3000000, operator));

    console.log("efury token address: ", efury.address);

    const bnAmount = web3.utils.toWei("100", "ether");

    await efury.mint(operator, bnAmount, setTxSpecifications(3000000, operator));

    if (network === "develop") {
      await efury.mint(accounts[1], bnAmount, setTxSpecifications(3000000, operator));
    }
